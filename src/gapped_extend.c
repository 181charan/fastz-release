...
// This controls the number of seeds that are launched
#define TOTBENCHMARKS 1000000

// Kernel declaration as they are defined externally.
...
static galign** init_from_anchors  (segtable* anchors, u64 numExtraSlots);
...
alignel* gapped_extend
   (seq*			seq1,
	...
	// holds the sequence 1 for right extension.
	char * charstr1 = (char*) malloc ( sizeof(char)*io.len1);
  	if (charstr1==NULL) {
		  exit (1);
	}

	// Holds the sequence 2 for left extension.
	char * charstr2 = (char*) malloc ( sizeof(char)*io.len2);
  	if (charstr2==NULL) {
		  exit (1);
	}

	// holds the reversed sequence 1 for left extension.
	char * revstr1 = (char*) malloc ( sizeof(char)*io.len1);
  	if (revstr1==NULL) {
		  exit (1);
	}

	// holds reversed sequence 2 for left extension.
	char * revstr2 = (char*) malloc ( sizeof(char)*io.len2);
  	if (revstr2==NULL) {
		  exit (1);
	}

	// Converts the input sequence into the 2-bit format 
	// and stores them  as a character array. While also
	// filling in the reverse array.
	// Only supports fasta files.

	char tempchar;
	for(int i=0;i<io.len1;i++){
		tempchar = (char)(alphaToInteger((char)(io.seq1[i])));
		charstr1[i] = tempchar;
		revstr1[io.len1-(i+1)] = tempchar;
	}


	for(int i=0;i<io.len2;i++){
		tempchar = (char)(alphaToInteger((char)(io.seq2[i])));
		charstr2[i] = tempchar;
		revstr2[io.len2-(i+1)] = tempchar;
	}
	...
	// Holds anchors
	anchor1array = (int*) malloc ( sizeof(int)*TOTBENCHMARKS);
  	if (anchor1array==NULL) {
		  exit (1);
	}

	anchor2array = (int*) malloc ( sizeof(int)*TOTBENCHMARKS);
  	if (anchor2array==NULL) {
		  exit (1);
	}

	// Holds left alignments terminations
	endl = (int*) malloc ( sizeof(int)*TOTBENCHMARKS);
  	if (endl==NULL) {
		  exit (1);
	}

	// Holds right alignments terminations
	endr = (int*) malloc ( sizeof(int)*TOTBENCHMARKS);
  	if (endr==NULL) {
		  exit (1);
	}

	// Array holds the scores
	scores = (int*) malloc ( sizeof(int)*TOTBENCHMARKS);
  	if (scores==NULL) {
		  exit (1);
	}

	// Holds the final alignments.
	finali = (int*) malloc ( sizeof(int)*TOTBENCHMARKS);
  	if (finali==NULL) {
		  exit (1);
	}

	int * OrgList = (int*) malloc ( sizeof(int)*TOTBENCHMARKS);
  	if (OrgList==NULL) {
		  exit (1);
	}
	...
	if(segBatches->len > 1){
		...
		fprintf(stderr,"[FASTZ] This proof of concept is not capable of dealing with more than one batches. !\n");
		exit(0);

	}

	// TOTBENCHMARKS define determins the total number of seeds processed in one batch.
	int benches = TOTBENCHMARKS;

	unsigned long long currentBinPerProblemResults; 
	...
	for (batIx=0 ; batIx<segBatches->len ; batIx++)
	{
		...
		// endSegIx currosponds to the total number of anchor points,
		// The GPU kernel launches TOTBENCHMARKS seeds at a time
		// Hence the seeds are processed in batches

		unsigned batchCnt = ceil((double)(endSegIx)/(double)TOTBENCHMARKS);
		
		//printf("[FASTZ][DEBUG]Total number of batches are %i\n",batchCnt);

		for(int outLoop = 0; outLoop < batchCnt; outLoop++)
		{	

			seedAccumulator = 0;
			ddone = 0;

			for (int b=startSegIx ; b<TOTBENCHMARKS ; b++)
			{
				if((b*batchCnt+outLoop)<endSegIx){
					mp = msp[(b*batchCnt+outLoop)];

					// skip processing this seed if it intersects with a previous alignment.
					if (!msp_left_right (orderBegInc, mp)){
						continue;
					}
							
					// accumulate seeds till you have enough to launch the kernel
					if(seedAccumulator < benches){
						anchor1array[seedAccumulator] = mp->pos1;
						anchor2array[seedAccumulator] = mp->pos2;
						endl[seedAccumulator] = 0;
						endr[seedAccumulator] = 0;
						scores[seedAccumulator] = 0;
						OrgList[seedAccumulator] = (b*batchCnt+outLoop); // stores the original anchor number, which is needed when passing this back to LastZ.
						seedAccumulator+=1;
					}

					if(seedAccumulator == benches){
						
						fprintf(stderr,"[FASTZ][DEBUG] Invoking inspector kernel with %i seeds!\n",seedAccumulator);
						kernel(charstr1, charstr2, revstr1, revstr2, io.len1, io.len2, anchor1array,anchor2array,scores,endl,endr,0, benches);
						ddone = 1;
					}
			...
			if(ddone == 0){
				fprintf(stderr,"[FASTZ][DEBUG] Invoking inspector kernel with %i seeds!\n",seedAccumulator);
				kernel(charstr1, charstr2, revstr1, revstr2, io.len1, io.len2, anchor1array,anchor2array,scores,endl,endr,0, seedAccumulator);
			}
			...
			// Of all the alignments found, find the maximum alignment length found
			// This information is used to allocate sufficient memory for the executor kernel
			for(int q=0;q<seedAccumulator;q++){

				t1 = ((  endr[q] >> 16) & 0x0000FFFF);
				t2 = ((  endr[q]) & 0x0000FFFF);
				...
				t1 = ((  endl[q] >> 16) & 0x0000FFFF);
				t2 = ((  endl[q]) & 0x0000FFFF);
				...
				// Only alignments who's scores are over the treshold matter
				if(scores[q] > scoreThresh.s){
			...
			currentBinPerProblemResults = (unsigned long long)(2*sizeof(char))*((unsigned long long)(maxF+maxF));

			tbResults = (char*) malloc ( ctot*currentBinPerProblemResults );
			if (tbResults==NULL) {
				exit (1);
			}

			// Launch the executor kernel
			kerneltb(charstr1, charstr2, revstr1, revstr2, io.len1, io.len2, anchor1array, anchor2array, maxF, scores, endl, endr, tbResults, finali, 1, seedAccumulator);

			int scoreCntr = 0;

			for (int c=0 ; c<seedAccumulator ; c++)
			{
				...
				editscript** scriptbb;
				editscript** scriptRightbb;

				// Extract the addresses of where the alignments are stored
				char * rext = &tbResults[(scoreCntr-1)*(2)*(maxF+maxF)+(maxF+maxF)];
				char * lext = &tbResults[(scoreCntr-1)*(2)*(maxF+maxF)];

				// LEFT alignments are the start of the alignment
				io.start1 = io.anchor1 + 1 - ((  endl[c] >> 16) & 0x0000FFFF) - 1;
				io.start2 = io.anchor2 + 1 - ((  endl[c]) & 0x0000FFFF) - 1;

				// RIGHT alignments are the end of the alignment
				io.stop1 = io.anchor1 + ((  endr[c] >> 16) & 0x0000FFFF) + 1;
				io.stop2 = io.anchor2 + ((  endr[c]) & 0x0000FFFF) + 1;
				...
				scriptRightbb = &scriptRight;

				// If the score is over the treshold 
				// only then process the alignments.

				if(scores[c] > scoreThresh.s){

					for(int k =0;k<((  finali[c] >> 16) & 0x0000FFFF);k++){

						if      (lext[k] == 1) {    	
							edit_script_ins (scriptbb, 1); 
						}
						else if (lext[k] == 2) { 
							edit_script_del (scriptbb, 1); 
						}
						else { 	
							edit_script_sub (scriptbb, 1); 
						}

					}		
		
					for(int k =0;k<(finali[c] & 0x0000FFFF);k++){

						if      (rext[k] == 1) {    	
							edit_script_ins (scriptRightbb, 1); 
						}
						else if (rext[k] == 2) { 
							edit_script_del (scriptRightbb, 1); 
						}
						else { 	
							edit_script_sub (scriptRightbb, 1); 
						}
						...
						// If score does not meet treshold just insert gaps.
						// Might not need to do this.

					int t1 = ((  endl[c] >> 16) & 0x0000FFFF) + 1;
					int t2 = ((  endl[c]) & 0x0000FFFF) + 1;
					...
					t1 = ((  endr[c] >> 16) & 0x0000FFFF) + 1;
					t2 = ((  endr[c]) & 0x0000FFFF) + 1;
				...
				// Internal LastZ function that formats the alignment.
...
static galign** init_from_anchors
   (segtable*	anchors,
	...
	u64			bytesNeeded, bytesPointers, bytesBlocks;
	galign**	snakes, *snake;
	segment*	anchor;
	...
	if ((u64max-16) / sizeof(galign) < (u64) numSlots) goto overflow1;
...
static void ydrop_align
   (alignio*	io)
	...
	fprintf(stderr,""unsposFmt","unsposFmt",",anchor1,anchor2);
	...
	//clock_gettime(CLOCK_MONOTONIC, &start);
		...
		//if(anchor1 == 12900856 && anchor2 == 13434716){

		
		//fprintf(stderr,"anchor1 is "unsposFmt" , anchor 2 is "unsposFmt" \n ",anchor1,anchor2);
		...
		//fprintf(stderr,""unsposFmt","unsposFmt","scoreFmt","unsposFmt","unsposFmt","scoreFmt" \n ",anchor1,anchor2,(scoreRight), end1, end2,(scoreRight+scoreLeft));					

		//fprintf(stderr,""unsposFmt","unsposFmt" \n ",anchor1,anchor2);
	//}

	// fprintf(stderr,"seq1: %s and seq2: %s\n",currParams->seq1Filename,currParams->seq2Filename);				// (from lastz.c)
	...
	//fprintf(stderr,"anchor1 is "unsposFmt" , anchor 2 is "unsposFmt" ",anchor1,anchor2);
	//fprintf(stderr,", and right score is "scoreFmt", right end is end1: "unsposFmt" end2: "unsposFmt", time for right extension is %lf, Total Score is: "scoreFmt" \n ",(scoreRight), end1, end2,time_taken,(scoreRight+scoreLeft));
...
static score ydrop_one_sided_align
   (alignio*	io,
	...
	// 	I[i] = (int *)malloc(len2+1 * sizeof(int));
	// 	D[i] = (int *)malloc(len2+1 * sizeof(int));
	// 	C[i] = (int *)malloc(len2+1 * sizeof(int));
	// }

	// //Just make sure that I/S/D[0][0] = 0 and not INT_MIN! 
	...
	//D[1][0] = -gapOE;
	*(tbp++) = 0;
	//fprintf(stderr,"(%i,%i):("scoreFmt","scoreFmt")\n",row,col,c,(dq)->DD);
	snoopAlgorithm_2;
	...
	for (col=1 ; (col<=N)&&(cTemp>=-yDrop) ; col++)
		...
		//D[1][col] = c - gapOE;
		//fprintf(stderr,"(C[%i],[%i]: "scoreFmt", D[%i][%i]: "scoreFmt"\n",row,col,c,row+1,col,c - gapOE);
		c -= gapE;
		*(tbp++) = cFromI;
		snoopAlgorithm_3;
		}
	...
	for (row=1; row<=M ; row++)
		...
		for ( ; (col<RY)&&((unspos)(b-B)<=N+1) ; col++)
			...
			// if( D[row][col] > C[row][col] || I[row][col] > C[row][col] )
			// {
			// 	if(D[row][col] >= I[row][col] ){
			...
			// 	I[row][col+1] = I[row][col] - gapE; 
			// 	D[row+1][col] = D[row][col] - gapE;

			// }
			// else
			// {
			// 	if(C[row][col] >= bestScoreMine){
			// 		bestScoreMine = C[row][col];
			// 		loc_i = (int)row;
			// 		loc_j = (int)col;
			// 	}

			// 	cTemp = C[row][col] - gapOE;
			// 	D[row+1][col] = D[row][col] - gapE;
			// 	if(cTemp > D[row+1][col]){
			// 		D[row+1][col] = cTemp;
			// 	}
			// 	I[row][col+1] = I[row][col] - gapE;
			// 	if( cTemp > I[row][col+1] ){
			// 		I[row][col+1] = (int)cTemp;
			// 	}
			...
			if ((d > c) || (i > c))			// === we CAN improve C ===
				...
				if (d >= i) { c = d;  link = cFromD | iExtend | dExtend; }
					...
					// 	//fprintf(stderr,"1:(%i,%i) c is "scoreFmt" , i is "scoreFmt" , d is "scoreFmt" , bestScore is "unsposFmt" \n",row,col,c,i,d,bestScore);
				...
				// 	//fprintf(stderr,"(%i,%i):(i:"scoreFmt",d:"scoreFmt")\n",row,col,i,d - gapE);
				...
				// //fprintf(stderr,"2:(%i,%i) c is "scoreFmt" , i is "scoreFmt" , d is "scoreFmt" , bestScore is "unsposFmt" \n",row,col,c,i,d,bestScore);
				...
				d -= gapE;					// set D[row+1][col]
				// if(row == 2 && col == 4){
				// 	fprintf(stderr,"D:(%i,%i):(i:"scoreFmt",d:"scoreFmt")\n",row,col,cOpen,d);
				// }
				...
				i -= gapE;					// 'set' I[row][col+1]
				// if(row == 1 && col == 1){
				// 	fprintf(stderr,"I:(%i,%i):(i:"scoreFmt",d:"scoreFmt")\n",row,col,i,d);
				// }
				if (cOpen > i) i = cOpen;
				          else link |= iExtend;
			...
			cTemp = sub[*(b++)];
			cNext = (dp++)->CC+cTemp;	// propose C[row][col+1]
						
						
			// This is the part I am not very CLEAR about!
			// The comments say that you are trying to propose
			// C[row][col+1] So you need the C[row-1][col]+S[row][col+1].
			//C[row][col+1] = C[row-1][col] + (int)cTemp;
			...
			// 	fprintf(stderr,"(%i,%i):(current: "scoreFmt",prevC: "scoreFmt", ADD: "scoreFmt" )\n",row,col,c,cNext-cTemp,cTemp);

			// }

			// if(row <= 159 && col <= 151){
			// 	fprintf(stderr,"(%i,%i):"scoreFmt":%i,",row,col,cTemp,(int)cTemp);
			// }	   
...
dp_finished:
	...
	*(_end2) = col = end2;
	...
	//fprintf(stderr,"DONE:(%i,%i) c is "scoreFmt" , bestScore is "unsposFmt" and its location is (%i,%i) \n",159,151,C[159][151], bestScoreMine, loc_i,loc_j);
...
#elif (defined debugPosB1)
	snoop = (anchor1 == debugPosB1) && (anchor2 == debugPosB2);
#endif // debugPosA1,debugPosB1#endif // debugPosA1,debugPosB1
